...
Consider the elements $M_{n, i} = \sigma_{i-1} \cdots \sigma_1 \sigma_1 \cdots \sigma_{i-1} \in BMW_n$ for $2 \leq i \leq n$ and set $M_{1,n}$ to be the identity. The set of $M_{n, i}$ in $BMW_n$ are called the \textbf{BMW Jucys-Murphy elements}, which are invertible and are known to be central (see \AP{Isaev-Ogievetsky}). 
\begin{figure}[h]
\centering
...
\end{figure}
The image of $\psi_n$ is clearly central. In the next proposition, we give a simple application which expresses $\psi_n(\tilde{P}_k)$ as a linear combination of the BMW Jucys-Murphy elements and their inverses. This is a Dubrovnik generalization of the HOMFLYPT counterpart found in \AP{Morton}.
