...
This map is not unital nor multiplicative, but it is a right $\cc$-module homomorphism. Furthermore, the closure $\cl_\ca(x \cdot e)$ is the element $\phi(x)$ which is the meridian map $\phi$ applied to $x$. The following identity is a generalization of Lemma 8.3 in Lukac. \AP{I think Shelly wrote about this. Make sure to cite him where necessary.}
...
\AP{Double check the $\upsilon_i^\pm$. Should they both be negative?}
\begin{align*}
\omega_{n}^+ &:= - \{n\} s^{1-n} v, \\
...
\AP{double check below. Make sure $\upsilon_i^\pm$ is right.}
\begin{align*}
...
Consider the elements $M_{n, i} = \sigma_{i-1} \cdots \sigma_1 \sigma_1 \cdots \sigma_{i-1} \in BMW_n$ for $2 \leq i \leq n$. The set of $M_{n, i}$ in $BMW_n$ are called the \textbf{BMW Jucys-Murphy elements}, which are invertible and generate a commutative subalgebra (see \AP{Isaev-Ogievetsky}). 
\begin{figure}[h]
\centering
...
\end{figure}
The image of $\psi_n$ is clearly central. In the next proposition, we give a simple application which expresses $\psi_n(\tilde{P}_k)$ as a linear combination of the BMW Jucys-Murphy elements, their inverses, and the identity. This is a Dubrovnik generalization of the HOMFLYPT counterpart found in \AP{Morton}.

\begin{proposition} \label{prop:murphy}
For any $k \geq 1$, we may represent $\psi_n(\tilde{P}_k)$ as
\begin{equation}
\psi_n(\tilde{P}_k) = \langle \tilde{P}_k \rangle \id_n + (s^k - s^{-k}) \sum_{i=2}^n v^{-k} M_{n, i}^k - v^k M_{n, i}^{-k}
\end{equation}
...
&=  \langle \tilde{P}_k \rangle M_{n,1} + (s^k - s^{-k}) \sum_{i=2}^n v^{-k} M_{n, i}^k - v^k M_{n, i}^{-k}
\end{align*}
\end{proof}
