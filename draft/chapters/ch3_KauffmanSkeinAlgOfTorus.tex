...
\end{enumerate}
Since $\eta_{(I \times I, [n])}$ is an algebra map, we just need to show that $\eta_{(I \times I, [n])}(\tilde{y}_n)$ is non-zero. If $n=1$, then the statement is true since $\tilde{y}_1$ and $JW_1$ are the simply the identity, a single strand. \AP{Induction with recurrence relation? or...}
...
\begin{center}
\begin{tikzcd}
	& BMW_n \arrow[dl, "{\pi_H}", two heads] \arrow[dd, "{\eta_{(I \times I, [n])}}", two heads] \\
