...
\end{enumerate}
Since $\eta_{(I^2, [n])}$ is an algebra map, we just need to show that $\eta_{(I^2, [n])}(\tilde{y}_n)$ is non-zero. If $n=1$, then the statement is true since $\tilde{y}_1$ and $JW_1$ are the simply the identity, a single strand. \AP{Induction with recurrence relation? or...}
...
\begin{center}
\begin{tikzcd}
	& BMW_n \arrow[dl, "{\pi_H}", two heads] \arrow[dd, "{\eta_{(I^2, [n])}}", two heads] \\
...
Recall that for a smooth and oriented surface $\Sigma$, we can define the HOMFLYPT and Dubrovnik skein categories $\sfh(\Sigma)$ and $\sfd(\Sigma)$. When $\Sigma=I^2$, these categories are symmetric monoidal, where the monoidal product is induced by some standardized embedding $m: I^2 \sqcup I^2 \to I^2$. Following \AP{BB01}, recursively define morphisms $\fa_\Lambda$ and $\fb_\Lambda$ by:
\begin{align*}
    \fa_1 &= \id_1 = \fb_1 
\end{align*}
...
\subsection{\textbf{The skein module $\cd(I^2 \times S^1)$.}} \label{sec:annulus}
...
We can transport these to to $BMW_n$ by defining $B_n := s_n(P_n)$. 
...
(\AP{Make sure this is what we want.})
%\begin{theorem}[\cite{MPS19}, Theorem 2.7]
%\[
%\cl(B_k) = \sum_{i+j+1=k} (-1)^j \hat{\tilde{y}}_{(i|j)}
%\]
%where $(i|j)$ is notation for the hook partition $\lambda = (i+1, 1, \dots, 1)$ with $j$ entries equal to $1$.
%\end{theorem}

\section{\textbf{The action of $\cd(T^2)$ on $\cd(I^2 \times S^1)$.}} \label{sec:action}
(\AP{Give a short description of the action.})
...
(\AP{Can we say anything else interesting about this action?})
